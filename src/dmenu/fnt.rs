...
use fontconfig::fontconfig::{
    FcBool, FcChar8, FcFontList, FcFontSet, FcFontSetDestroy, FcNameParse, FcObjectSetBuild,
    FcObjectSetDestroy, FcPatternDestroy, FcPatternGetBool, FcResultMatch,
};
use libc::c_uint;
use std::ffi::c_void;
...
use std::ptr;
use x11::xft::{
    FcPattern, XftFont, XftFontClose, XftFontOpenName, XftFontOpenPattern, XftNameParse,
};
...
impl Fnt {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            if FcPatternGetBool((*xfont).pattern as *mut c_void, FC_COLOR, 0, &mut iscol)
                == FcResultMatch
                && iscol != 0
            {
                XftFontClose(drw.dpy, xfont);
    ...
    fn find_font_sys(needle: &String) -> Result<(), String> {
        ...
        unsafe {
            ...
            if (*fs).nfont == 0 {
                // user may have searched for an attribute instead of family
                FcFontSetDestroy(fs);
                fs = Self::op_pattern(format!(":{}", searchterm))?;
            }
            let ret = if (*fs).nfont == 0 || (*fs).nfont == (*all_fonts).nfont {
                Err(format!(
                    "Warning: font '{}' not found on the system",
                    searchterm
                ))
            } else {
                Ok(())
